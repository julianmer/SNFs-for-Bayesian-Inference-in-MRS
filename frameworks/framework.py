...
#**************************************************************************************************#
class Framework():
    def __init__(self, path2basis, basisFmt='', specType='all', dataType='none', ppmlim=None):
        ...
        if dataType == 'clean': self.ps, self.concs = cleanParams, normConcs
        ...
        elif dataType == 'const': self.ps, self.concs = constParams, meanConcs
        elif dataType == 'challenge': self.ps, self.concs = challengeParams, challengeConcs
        elif dataType == 'mrsi': self.ps, self.concs = mrsiParams, normConcs
        elif dataType == 'aumc': self.ps, self.concs = aumcParams, aumcConcs
        elif dataType == 'aumc2': self.ps, self.concs = aumcParams2, aumcConcs
        elif dataType == 'aumc_ms': self.ps, self.concs = aumcParams, aumcConcsMS
        elif dataType == 'aumc_mod': self.ps, self.concs = aumcParamsMod, normConcs
        elif dataType == 'aumc_mod2': self.ps, self.concs = aumcParamsMod2, normConcs
        elif dataType == 'aumc_mod_lw': self.ps, self.concs = aumcParamsModLW, normConcs
        elif dataType == 'aumc_rw_p': self.ps, self.concs = aumcParamsRWP, aumcConcs
        else: self.ps, self.concs = None, None

        self.basisObj = Basis(path2basis, fmt=basisFmt)
