...
class FrameworkLCModel():
    def __init__(self, path2basis, control=None, multiprocessing=False, ppmlim=(0.5, 4.2),
                 ...
                 bandwidth=None, verbose=1, **kwargs):
        ...
        if bandwidth is None: self.bandwidth = self.basisFSL.bandwidth
        ...
        if control is not None:
            ...
            lines.append(f"filtab='example.table'")

            lines.append(f"lcoord=9")  # 0 <-> surpress creation of coord file, 9 <-> don't surpress
            lines.append(f"ltable=7")  # 0 <-> don't create table file, 7 <-> create table file
    ...
    def lcmodel_minimize(self, x, x_ref=None, frac=None):
        ...
        if self.save_path == '': path = os.getcwd() + os.sep + 'tmp' + os.sep
        else: path = os.getcwd() + os.sep + self.save_path + os.sep
    ...
    def initiate(self, file_path):
        ...
        for i, line in enumerate(self.control):
            ...
            if line.startswith('filtab='): self.control[i] = f'filtab=\'{file_path[:-4]}.table\''
...
if __name__ == '__main__':
    ...
    config = {
        'path2basis': '../Data/press3T_30ms.BASIS',
        ...
        'sample_points': 2048,   # if basis has different number of points than the samples
    }

    lcm = FrameworkLCModel(config['path2basis'], sample_points=config['sample_points'])
    ...
    concs = [[c[met] for met in lcm.basisFSL._names] for c in concs]   # sort by basis names
    concs = np.array(concs)[:, :lcm.basisFSL.n_metabs]
    ...
    # fit
    lcm.save_path = './tmp/'
    thetas, uncs = lcm(data, water)  # data in freq. domain, shape of (batch_dim, 2, sample_points)
                                           # water in time domain, shape of (batch_dim, sample_points)
    # loss
    if 'path2water' not in config: thetas = lcm.optimalReference(concs, thetas) * thetas
